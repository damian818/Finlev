...
async function startServer() {
  ...
  app.get("/api/inflation-fx-history", async (req, res) => {
    try {
      ...
      // Filter and aggregate monthly records from 2024 to present
      const monthlyFx: Record<string, number> = {};
      fxData.forEach(item => {
        const month = item.fecha.substring(0, 7);
        // keep latest available rate for each month
        monthlyFx[month] = item.venta || item.compra;
      });

      // Filter inflation from 2024-01 onwards
      ...
      const historyPoints = recentInfl.map((item, idx) => {
        ...
        // Manual Overrides for simulation months (2026) based on user provided historical series
...
Provide 3 actionable financial recommendations, 2 key spending risks or anomalies, and a brief overall financial health score (0-100) with a 2-sentence summary. Format your response in clear JSON structure or clean markdown.`;
      ...
      if (error?.message?.includes('resource_exhausted') || error?.status === 429) {
        return res.status(429).json({ error: "AI quota temporarily exceeded. Please try again shortly or check your API plan." });
      }
...
Answer the user's questions clearly, accurately, and concisely. If they ask about spending comparisons (e.g. "Am I spending more in restaurants than before?", "Where am I spending more?"), reference the actual numbers from their financial data provided above.`;
      ...
      if (error?.message?.includes('resource_exhausted') || error?.status === 429) {
        return res.status(429).json({ error: "AI quota temporarily exceeded. Please try again in a few moments." });
      }
